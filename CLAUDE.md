...
`README.md` is the user-facing overview; this file is the guide for editing the code.
...
**This file is RESIDENT (every session pays for it); `.claude/rules/*.md` are FETCHED per layer and digested into every convention judge; `docs/*.md` are REFERENCED only by a session following a pointer.**

A constraint any edit could violate belongs here; one that bites inside a layer belongs in that layer's file; how to run, install or test belongs in `docs/`.
...
| Run pipeline — plan gate, ladder, verify, supervisor | `workflows.py`+`wf_*.py`, `worker.py`, `activities.py`, `judging.py`, `plans.py`, `conventions.py`, `supervisor.py` | `.claude/rules/run-pipeline.md` |
...
| Gateway, backends, walls | `gateway.py`, `local_runtime.py`, `error_classifier.py` | `.claude/rules/gateway-backends.md` |
| Tool grants, MCP, file safety, sandbox | `mcp_client.py`, `policy.py`, `claude_cli.py`, `file_safety.py` | `.claude/rules/tools-mcp.md` |
...
- **Workflow code must never call `config.setting()`** — the store is mutable, so a replay could branch differently than history recorded. `OttoWorkflow` takes ONE snapshot via `snapshot_settings` and reads it through `self._setting(...)` (`ResidentRuleGuardTests`).
...
- **SQLite stores go through `storage.sqlite_connect`+`storage.tx`** (all tables in `data/otto.db`) — one connection per operation; they aren't thread-safe and `server.py` is threaded (`ResidentRuleGuardTests`).
- **Any read-modify-write must use `storage.tx` (`BEGIN IMMEDIATE`)** — `BEGIN DEFERRED` lets concurrent writers upgrade-fail and lose writes silently. Keep `gateway.embed` outside it; embeddings are packed float32 BLOBs (`knowledge._pack`) (`ResidentRuleGuardTests`).
...
Anything a `grep` answers in two seconds is deliberately absent. For the non-obvious homes, the layer table above is the index:

Admin writes + conventions UI → `ui.md`; stock caps + risk model → `routing-capabilities.md`; scorecard + retry/accept → `memory-privacy.md`, `repo-work.md`; the `engine.X` facade and audit store → `engine-core.md`.
