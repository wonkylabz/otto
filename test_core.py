...
class ClaudeMdBudgetTests(unittest.TestCase):
    ...
    # 83009 -> 83690 (#129): stage timings are now on the audit row. Three rules, and each one
    # is a wrong answer a later reader reaches by default: the map is run-level but lands on
    ...
    # 83690 -> 85044 (#128): the trace log became durable, so it is now a sink with the same
    # obligations a transcript has. Six rules, each one measured: the deny globs do not reach a
    # subdirectory (and read-deny alone let a run erase itself), a day stamp bounds nothing on a
    ...
    # 85044 -> 85321 (#127): the chat was blind for the whole 900s plan preview. One rule, and
    ...
    # 86434 -> 86701: the models list is dragged into order. One rule, and the half that cost a
    # debugging pass is the `<colgroup>`: the widths live on the `<col>`s, so a `th` added without
    # its `<col>` silently shifts every column after it onto the previous one's width.
    MAX_RULES_BYTES = 86701   # fetched tier — bounded, but looser; it is not always loaded
    MAX_RULE_CHARS = 280
