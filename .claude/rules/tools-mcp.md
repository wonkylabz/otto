...
- **A def's credential is RESOLVED into `data/.mcp-active.json` (0600, read-denied), never left as `${VAR}`** — the env is the only other lane and the run's own Bash inherits it, so wiring ONE server published that token to every run (`McpConfigResolutionTests`).
...
- **`slack_mcp.py` imports NOTHING of Otto's** — it is spawned with `OTTO_*` stripped, so `config.secret` would find nothing and Temporal would load per run; its token arrives as the def's own `SLACK_TOKEN` (`SlackAdminMcpTests`, `SlackAdminMcpSpawnTests`).
