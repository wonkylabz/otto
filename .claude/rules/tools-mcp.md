...
- **UNDECLARED = the REQUEST picks the fleet, and a pointer can't** — "work on this ticket \<url\>" matched one stopword, so a New Relic task got 2 k8s tools and 0 of 38 NR ones. Hence `_STOP`/`_URL` in `_words` (`LocalMcpToolBudgetTests`).
