...
- **Repo-conventions injection** (`conventions.py`) — the judge is a bare `gateway.complete` with no cwd, so it gets a digest of the target repo's CLAUDE.md; the executor gets this free via `claude -p`'s cwd. Cached in `data/conventions.json`, keyed on mtime+size.
  - Conventions override the request: a faithfully-implemented rule violation is a FAIL.
  - **Ranked against the request, never truncated in document order** — first-that-fit makes the enforced subset a function of section order, so reordering CLAUDE.md swaps what the judge enforces (`ConventionsBudgetTests`).
  - `select_rules` states how many rules it dropped; a silent trim reads as "those were all the rules". The cache holds the FULL extraction, and `_CACHE_V` forces re-derivation.
...
- **A tool is in the grant only once it RETURNS** (`_grant_list`/`_refused_note`) — `READ_TOOLS` omits a cap's own `tools:` and inherited connectors, so a judge reading it as closed fails real connector work as fabricated (`JudgeToolGrantTests`).
- **A mere attempt is not a grant** — an ungranted connector refuses every call, so crediting the attempt turns a truthful "source blocked" into an invented excuse (`JudgeToolGrantTests`).
...
- **Unattended dead-end rule** — with nobody present, a report whose bottom line is a question is a FAIL. `audience="slack"` *replaces* it (a question back is legitimate there), failing leaked scaffolding, wrong-audience replies and "check this yourself".
