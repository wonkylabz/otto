...
- **`provider` is TRANSPORT; `gateway.model_kind` is CLASS** — `kind` is stored on the endpoint (`local`/`hosted`, guessed if unset). Latches and failure copy read the class; dispatch, `_default_claude` repoints and the Claude-only `preview` are transport (`ModelKindTests`).
