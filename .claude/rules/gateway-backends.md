...
- **No fallback lands on haiku** — `_default_claude` is sonnet-first. 'First entry' meant surprise opus; 'cheapest' made haiku the silent answer wherever local can't serve, the preview included. `downshift_model_id` keeps cheapest: an opted-in spend lever, not a fallback.
- **"Auto" is a per-run SENTINEL on `model_override`, not a pool entry** (`gateway.is_auto`) — every override is validated against the pool, so a hop not admitting it explicitly 400s or drops it and the option is inert (`AutoExecTierTests`).
- **Auto is also the ADMIN default** (`assign.execution="auto"`, the Auto row) — the one non-pool assignment value, execution-only; `_model_for` maps it to the default Claude entry so backends and fallbacks see a real model; a composer pick wins (`AdminAutoExecTests`).
- **Auto sits BELOW `cap_exec` and decides attempt 1 only** — a per-cap pin is a statement about that cap, and re-classifying a retry fights the escalation. Never on local: that moves a cost opt-out to Claude (`AutoExecTierLadderTests`).
- **An auto pick resolves UPWARD, never onto an unhealthy model** (`gateway.auto_model_id`) — cheaper tiers were already judged insufficient, and auto has no human behind it to notice a broken one. Nothing healthy at/above ⇒ None, the normal pick stands (`AutoExecTierTests`).
- **Model health is real-outcome-first** (`gateway.record_health`/`unhealthy_models`, `/api/health`) — every call records ok/failed, last-write-wins, so a model nothing retries still warns. Only "cannot serve any run" failures count (unreachable, mis-configured, tool-calls-rejected), or the badge means nothing. Probing covers configured-but-idle models only; Claude tiers only on `force=True`.
...
- **This is ONE Admin control, not three** — `cap_exec`/`cap_local_exec`/policy `tool_free` interact invisibly: tool-free is checked *first*, so setting both meant attempt 1 had no tools *and* every later rung was pinned local, no Claude backstop. One `Execution` select (`execSelect`), one writer (`setCapBackend`).
...
- **Claude fallback is a flag** (`config.LOCAL_FALLBACK`, default on) — a failing local model is covered by Claude; `=0` (strict) forbids that: the run stops and delivers `config.strict_stop_message`. `verify` is exempt (the judge must catch a bad local execution, not die with it). Strict-stop is terminal in every ladder.
- **A DETERMINISTIC local wall latches the ladder off local; a budget death does not.** Tool-calls-rejected and endpoint-unreachable (`local_runtime`'s `tools_unsupported`/`unavailable` → `engine`'s `local_wall`) fail identically every attempt, so the run re-dispatches to Claude; a turn-budget death is the model working and not finishing, so a retry with the critique stays local (`…redispatches_instead_of_burning_the_ladder`/`…does_NOT_latch_the_ladder`).
...
- **`--allowedTools` grants permission but unloads nothing** — every built-in, MCP server, skill and agent stays in every turn's system prompt. `--disallowedTools` on the complement (`config.DISALLOWED_TOOLS`, synced with `KEEP_TOOLS` by `ContextTrimTests`) is what removes them. **Never disallow `ToolSearch`** — deferred MCP schemas load through it.
