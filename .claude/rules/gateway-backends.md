...
- **A body adaptation only ever removes or renames, and one nothing can fix WALLS** — a two-directional rewrite oscillates until the round budget dies, reporting the wrong cause; a refusal no rewrite fixes is `tools_unsupported` → Claude (`OpenAiParamDialectTests`).
...
- **PLAN is the ONE place local Bash IS scoped** (`config.scoped_bash_rules` → `local_runtime.bash_refusal`) — `--permission-mode plan` has no local equivalent, and the alternatives were no Bash (planning a ticket blind) or unscoped shell before approval (`LocalPlanModeTests`).
- **That scoping is an ALLOWLIST — why it works where `_deny_guard`'s Bash cannot** — refuse every shell metacharacter (the rest is argv, not a shell), then match the argv prefix TOKEN-wise. No rules refuses everything; unscoped runs pass `bash_rules=None` (`LocalPlanModeTests`).
