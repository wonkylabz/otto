...
- **Every `/chat/completions` body comes from `gateway.chat_body`** — the parameter names are the ENDPOINT's (OpenAI's newer models reject `max_tokens` and a non-default `temperature`), learned from the server's own 400 and remembered on the entry (`OpenAiParamDialectTests`).
