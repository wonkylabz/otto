...
- **The live progress tail covers the PLAN preview, and its phase flag is DERIVED** (`planActive` ← `times.PLAN`) — the workflow reports plain `running` for the whole 900s pass, and a latched flag paints the finished preview over a live attempt (`PlanVisibilityTests`).
