...
memory (facts), solutions (verified-pass approaches, cap 200), behaviors (advisory user directives, cap 50, never a security control), knowledge (chunked+embedded docs, RAG or keyword fallback), audit (`audit`+`audit_content`, immutable).

- **Stale secondary sources are the standing failure mode for question-shaped runs.** `_REPORT_FORMAT` demands a primary source (repo/cluster/cloud) for a current-state claim, forbids inferring non-existence from absence in a note, and dates any secondary-only answer.
...
- **The assistant cap's prompt and the memory-context header must agree**: stable facts (how something works, past decisions) answer from context; current-state facts (exists/deployed/enabled) re-check with tools; the tool wins. A contradiction re-serves one bad fact forever.
...
`contracts.py` — every invocation carries guidance on *how* to report: a labelled `**TLDR**` line, forbidding Otto's own vocabulary ("the gate", "AC #1").

A `**What you need to do**` line follows, omitted when nothing is needed — or every clean run grows a footer that trains the eye past the one that matters.

It lives in `_TLDR_SHAPE`, interpolated into `_REPORT_FORMAT`/`_SINGLE_TURN_CONTRACT`/`_RESUME_CONTRACT`.

- **A run NOTE never rides out to a CONVERSATION** (`workflows._shape_result`) — workspaces, verify rounds, Admin screens and the needs-human banner are Otto's vocabulary, appended AFTER the reply is written, so no contract polices them (`FreshRouteWriteGateTests`).
...
- **Who reads the result picks the contract** (`delivery.AUDIENCE`→`engine._output_contract`): `conversation` (a person who replies) gets `_DIRECT_REPLY_FORMAT`, `report` (a durable record) gets `_REPORT_FORMAT`. A new reply-target kind declares its audience; unknown → `report`.
...
- **A conversational run can choose to say nothing** (`config.is_no_reply`) — filler has no keyword-safe detection, so the model decides; a strict sentinel-match lets `verify` pass it and `delivery._slack` post nothing. A silent turn binds the session, never `last_reply`.
...
- **Per-stage `times` is a RUN-level map on ATTEMPT rows** — merge across a run's rows, closed span winning (`audit.run_times`): an attempt is audited from INSIDE the RUN span, and a clean run writes no terminal row to close it (`StageTimingTests`).
- **`stage_timings` shares `scorecard`'s judged-runs denominator** — a resume has no pre-attempt chain at all, so counting it drags every stage median toward zero (`StageTimingTests`).
