...
- **Tier calls are booked by the gateway, not the trail** — only attempts reach `audit`, so `gateway.complete` writes a `ledger` row per call: cost + the caller's `decided()`, never prompt/reply. A bare `_claude_complete` books nothing (`TierCallLedgerTests`).
