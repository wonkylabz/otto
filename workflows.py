...
with workflow.unsafe.imports_passed_through():
    ...
    from activities import (clarify_request, classify_followup, classify_request,
                            ...
                            resolve_pinned_cap,
                            notify_human, plan_capability, plan_swarm, open_chat,
                            plan_task_steps, poll_board, poll_pr_reviews, poll_slack,
                            provision_workspace, resolve_pr_target, check_grounding,
                            interim_notice, reap_stuck,
                            distil_memory, record_attempt, record_chat, record_skip,
                            route_request,
                            run_capability, snapshot_repos, snapshot_settings, suggest_repo,
                            verify_capability)
    # The pipeline's own call options and terminal copy, and the three mixins OttoWorkflow is
    # composed from. All deterministic workflow code, so they are passed through rather than
    # re-imported into the sandbox behind the class that inherits them (issue #58). The
    # underscored names are re-bound here because `workflows._EXEC_CEILING` is what the guard
    # tests and the rest of this module address them by.
    from wf_runtime import (_EXEC_CEILING, _HEARTBEAT, _JUDGE_CEILING, _NEEDS_HUMAN_BANNER,
...
class OttoWorkflow(RepoFlowMixin, PostPrMixin, SwarmMixin):
    ...
    workspace tail (`wf_repo`), the post-PR review/QA fix loops (`wf_postpr`) and swarm fan-out
    (`wf_swarm`). They are ordinary methods on this class at runtime — the split is about which
    file a bug in them gets fixed in, not about the run's shape."""
